...
st.sidebar.header('Enter Your Detalis')
...
def CreateStory():
    st.header('Welcome to the Story Maker... ', name)
    st.subheader("A Random Story...")

    st.write(random.choice(intros))
    st.write(random.choice(characters))
    st.write(random.choice(problems))
    st.write(random.choice(solutions))
    st.write(random.choice(endings))

st.button('Create A New Story',CreateStory())
